...
class ServerConnection:
    ...
    _KEINE_RUECKFALL_SCHLUESSEL = frozenset({
        ...
        "rcon_port", "zones", "scheduled_tasks", "shop_items_file", "types_xml_path",
...
class DayZBot(discord.Client):
    ...
    # ── Auto-Aufgaben (Dashboard-Scheduler, siehe _scheduled_tasks) ──
    @tasks.loop(seconds=30)
    async def scheduled_tasks_loop(self):
        try:
            await self._scheduled_tasks_once()
        except Exception as e:
            log.error(f"[AUTO-AUFGABEN] Fehler: {e}")

    async def _scheduled_tasks_once(self):
        for conn in connections.all():
            try:
    ...
    async def _scheduled_tasks_conn(self, conn: ServerConnection):
        ...
        if not faellig:
            return
        for task in faellig:
            ok_, msg, eigenes = await self._scheduled_task_ausfuehren(conn, task)
            log.info(f"[AUTO-AUFGABEN] {conn.name}: '{task.get('task')}' ausgeführt: "
                     f"ok={ok_} – {msg}")
            if not task.get("channel_ignore") and task.get("channel_id"):
                if ok_ and eigenes is not None:
                    # Die Aufgabe IST die Nachricht (z. B. Online-Liste) –
                    # eine Quittung "✅ ausgeführt" waere hier nur Rauschen.
                    e = eigenes
                else:
                    meta = SCHEDULED_TASK_TYPES.get(task.get("task"), {})
                    e = discord.Embed(
                        title=("✅ " if ok_ else "❌ ") + meta.get("label", str(task.get("task"))),
                        description=(f"Fehler: {msg}" if not ok_ else (task.get("note") or None)),
                ...
                if ch is not None:
                    try:
                        ...
                        log.error(f"[AUTO-AUFGABEN] Channel-Versand fehlgeschlagen: {ex}")
            task["next_execution"] = _scheduled_task_vorwaerts(
                float(task.get("next_execution", jetzt)),
                int(task.get("interval_seconds", 60)), jetzt)
        _scheduled_tasks_save(conn)

    async def _scheduled_task_ausfuehren(
            self, conn: ServerConnection,
            task: Dict) -> Tuple[bool, str, Optional[discord.Embed]]:
        """Fuehrt eine Aufgabe aus: ``(geklappt, Meldung, eigenes Embed)``.

        Ist das dritte Feld gesetzt, postet der Aufrufer DIESES Embed statt
        der allgemeinen Ausgefuehrt-Bestaetigung.
        """
        art = task.get("task")
        if art == "restart_server":
            if conn.api is None:
        ...
        schaden = SCHEDULED_TASK_TYPES.get(art, {}).get("schaden")
        if schaden:
            # Laeuft durch denselben Helfer (und damit dasselbe Lock) wie
            # /change_damage_settings – sonst koennten ein Befehl und eine
            # gleichzeitig faellige Aufgabe einander ueberschreiben.
            schluessel, neuer_wert = schaden
            code, infos = await _schaden_datei_setzen(conn, schluessel, neuer_wert)
            # Kein _t: eine Auto-Aufgabe hat keinen ausloesenden Nutzer, dessen
            # Discord-Sprache man fragen koennte – proaktive Posts bleiben deutsch.
            an_aus = "AUS" if neuer_wert else "AN"
            meldungen = {
                "ok": f"{schluessel} = {str(neuer_wert).lower()} (Schaden {an_aus}) – "
                      f"wirkt nach dem nächsten Server-Neustart",
                "unveraendert": f"stand bereits auf {an_aus} – Datei nicht angefasst",
                "kein_ftp": "Kein FTP-Zugang für diesen Server.",
                "kein_pfad": "Mission-Ordner unbekannt – /ftp_scan nötig.",
                "lesefehler": "cfggameplay.json nicht lesbar (FTP-Fehler) – nichts geändert.",
                "fehlt": "Keine cfggameplay.json im Mission-Ordner – nichts geändert.",
                "kaputt": f"cfggameplay.json ist kein gültiges JSON: {infos['fehler']}",
                "kein_abschnitt": f"{schluessel} steht nicht in der cfggameplay.json "
                                  f"und es gibt keinen Abschnitt GeneralData.",
                "schreibfehler": "Schreiben per FTP fehlgeschlagen – Datei unverändert.",
            }
            geklappt = code in ("ok", "unveraendert")
            meldung = meldungen.get(code, code)
            if not geklappt:
                return False, meldung, None
            log.info(f"[GAMEPLAY] {conn.name}: {schluessel} = "
                     f"{str(neuer_wert).lower()} (Auto-Aufgabe)")
            # Eigenes Embed statt der schlichten ✅-Quittung: der Hinweis auf
            # den noetigen Neustart ist bei einer Aufgabe, die nachts allein
            # laeuft, wichtiger als beim Befehl – dort steht ja ein Admin davor.
            label = SCHEDULED_TASK_TYPES[art]["label"]
            e = discord.Embed(
                title=f"🧱 {label}",
                description=str(task.get("note") or "") or None,
                color=0x2ECC71 if not neuer_wert else 0xE67E22,
                timestamp=datetime.now(timezone.utc))
            e.add_field(name="Ergebnis", value=meldung, inline=False)
            e.set_footer(text=f"⚠️ Wirkt erst nach einem Server-Neustart · {conn.name}")
            return True, meldung, e
...
#
# ``channel_pflicht``: Aufgaben, die AUS der Nachricht bestehen statt eine
# Aktion zu quittieren. Ohne Channel liefen sie ins Leere, deshalb laesst sich
# "Channel ignorieren" dort nicht setzen.
# ``schaden``: (Schluessel in der cfggameplay.json, neuer Wert). Der Wert ist
# das VERBOT, nicht die Erlaubnis – "Schaden an" heisst disableXxxDamage=False.
SCHEDULED_TASK_TYPES: Dict[str, Dict[str, Any]] = {
    "restart_server": {"label": "Server neu starten"},
    "online_list": {"label": "Online-Liste posten", "channel_pflicht": True},
    "base_damage_on":       {"label": "Base Damage anschalten",
                             "schaden": ("disableBaseDamage", False)},
    "base_damage_off":      {"label": "Base Damage ausschalten",
                             "schaden": ("disableBaseDamage", True)},
    "container_damage_on":  {"label": "Container Damage anschalten",
                             "schaden": ("disableContainerDamage", False)},
    "container_damage_off": {"label": "Container Damage ausschalten",
                             "schaden": ("disableContainerDamage", True)},
}
...
def _online_liste_embed(conn: ServerConnection, namen: List[str],
                        notiz: str = "") -> discord.Embed:
    """Wer ist gerade online? Kopfzeile mit Anzahl, darunter die Namen.

    Die Namen kommen aus dem Spiel und sind Fremdeingabe: ohne
...
def _scheduled_tasks(conn: ServerConnection) -> List[Dict]:
    """Die geplanten Aufgaben EINES Servers – nie ein Rueckfall auf einen
    anderen Server oder die globale config, siehe _KEINE_RUECKFALL_SCHLUESSEL."""
    ts = conn.data.get("scheduled_tasks")
    if not isinstance(ts, list):
        ts = []
        conn.set("scheduled_tasks", ts)
    return ts


def _scheduled_tasks_save(conn: ServerConnection) -> None:
    _conn_store(conn, "scheduled_tasks", _scheduled_tasks(conn))


def _find_scheduled_task(conn: ServerConnection, task_id: int) -> Optional[Dict]:
    for t in _scheduled_tasks(conn):
        if isinstance(t, dict) and int(t.get("id", 0)) == task_id:
            return t
    return None


def _naechste_scheduled_task_id(conn: ServerConnection) -> int:
    vorhandene = [int(t.get("id", 0)) for t in _scheduled_tasks(conn) if isinstance(t, dict)]
    return 1 + max(vorhandene or [0])


def _scheduled_task_vorwaerts(next_execution: float, interval_seconds: int,
                              jetzt: Optional[float] = None) -> float:
    """Naechsten faelligen Zeitpunkt ab next_execution finden – bei einer
    laengeren Ausfallzeit des Bots wird so uebersprungen statt die Aufgabe
    fuer jeden verpassten Zyklus auf einmal nachzuholen."""
    jetzt = jetzt if jetzt is not None else time.time()
    schritt = max(60, int(interval_seconds))
    while next_execution <= jetzt:
        next_execution += schritt
...
def _scheduled_task_payload(t: Dict) -> Dict[str, Any]:
    meta = SCHEDULED_TASK_TYPES.get(t.get("task"), {})
    return {
        "id": t.get("id"), "task": t.get("task"),
        "task_label": meta.get("label", t.get("task")),
        "initial_execution": t.get("initial_execution"),
        "next_execution": t.get("next_execution"),
        "interval_seconds": t.get("interval_seconds"),
        "note": t.get("note", ""),
        "channel_id": t.get("channel_id"),
        "channel_ignore": bool(t.get("channel_ignore")),
        "channel_pflicht": bool(meta.get("channel_pflicht")),
    }


def _scheduled_task_aus_anfrage(data: dict, vorhanden: Optional[Dict] = None):
    """Liest/validiert die Felder aus dem Formular. Gibt (werte, fehler)
    zurueck – bei einem Fehler ist werte None und fehler ein Text."""
    task = str(data.get("task", (vorhanden or {}).get("task", ""))).strip()
    if task not in SCHEDULED_TASK_TYPES:
        return None, "Unbekannter Aufgaben-Typ."

    try:
        initial = float(data["initial_execution"]) if "initial_execution" in data \
            else float((vorhanden or {})["initial_execution"])
    except (KeyError, TypeError, ValueError):
        return None, "Zeitpunkt der ersten Ausführung fehlt oder ist ungültig."

    try:
        interval = int(data["interval_seconds"]) if "interval_seconds" in data \
            else int((vorhanden or {})["interval_seconds"])
    except (KeyError, TypeError, ValueError):
        return None, "Intervall fehlt oder ist keine Zahl."
    if interval < 60:
        return None, "Intervall muss mindestens 60 Sekunden betragen."

    note = str(data.get("note", (vorhanden or {}).get("note", ""))).strip()[:200]
    channel_ignore = bool(data.get("channel_ignore", (vorhanden or {}).get("channel_ignore", False)))
    channel_id = data.get("channel_id", (vorhanden or {}).get("channel_id"))
    if SCHEDULED_TASK_TYPES[task].get("channel_pflicht"):
        # Diese Aufgabe besteht aus der Nachricht selbst – "Channel ignorieren"
        # waere gleichbedeutend mit "gar nichts tun".
        channel_ignore = False
        if not channel_id:
            return None, ("Für diese Aufgabe ist ein Channel Pflicht – "
                          "sie besteht aus der Nachricht selbst.")
    elif not channel_ignore and not channel_id:
        return None, "Bitte einen Channel wählen oder „Channel ignorieren“ aktivieren."

    return {
        "task": task, "initial_execution": initial, "interval_seconds": interval,
        "note": note, "channel_ignore": channel_ignore,
        "channel_id": None if channel_ignore else str(channel_id),
    }, None


async def list_scheduled_tasks(request: web.Request) -> web.Response:
    _c, denied = _session_conn(request)
    if denied is not None:
        return denied
    return ok({"tasks": [_scheduled_task_payload(t) for t in _scheduled_tasks(_c)],
               "task_types": [{"key": k, "label": v["label"],
                              "channel_pflicht": bool(v.get("channel_pflicht"))}
                             for k, v in SCHEDULED_TASK_TYPES.items()]})


async def create_scheduled_task(request: web.Request) -> web.Response:
    _c, denied = _session_conn(request)
    if denied is not None:
        return denied
    data = await body(request)
    ...
    if fehler:
        return err(fehler)
    if not werte["channel_ignore"] and _c.guild_id is not None:
        denied = _kanal_gehoert_guild(int(_c.guild_id), int(werte["channel_id"]))
        if denied is not None:
            return denied

    task = dict(werte)
    task["id"] = _naechste_scheduled_task_id(_c)
    task["next_execution"] = task["initial_execution"]
    _scheduled_tasks(_c).append(task)
    _scheduled_tasks_save(_c)
    return ok(_scheduled_task_payload(task))


async def update_scheduled_task(request: web.Request) -> web.Response:
    _c, denied = _session_conn(request)
    if denied is not None:
        return denied
    try:
    ...
    task = _find_scheduled_task(_c, task_id)
    if not task:
        return err("Aufgabe nicht gefunden.", 404)
    data = await body(request)
    werte, fehler = _scheduled_task_aus_anfrage(data, task)
    if fehler:
        return err(fehler)
    if not werte["channel_ignore"] and _c.guild_id is not None:
        denied = _kanal_gehoert_guild(int(_c.guild_id), int(werte["channel_id"]))
    ...
    # Aendert sich der Zeitpunkt/das Intervall, gilt der neue Plan ab jetzt -
    # sonst koennte eine gerade erst bearbeitete Aufgabe sofort (erneut) feuern.
    zeit_geaendert = (werte["initial_execution"] != task.get("initial_execution")
                      or werte["interval_seconds"] != task.get("interval_seconds"))
    task.update(werte)
    if zeit_geaendert:
        task["next_execution"] = werte["initial_execution"]
    _scheduled_tasks_save(_c)
    return ok(_scheduled_task_payload(task))


async def delete_scheduled_task(request: web.Request) -> web.Response:
    _c, denied = _session_conn(request)
    if denied is not None:
        return denied
    try:
    ...
    if not task:
        return err("Aufgabe nicht gefunden.", 404)
    _scheduled_tasks(_c).remove(task)
    _scheduled_tasks_save(_c)
...
def _rechtsseite(lang: str, titel: str, zurueck: str, inhalt_html: str) -> str:
    """Eigenstaendige, dunkel gestaltete Seite fuer Impressum/Datenschutz/
    Nutzungsbedingungen – bewusst AUSSERHALB der SPA (kein JavaScript
    noetig), damit sie immer und von jeder Unterseite in maximal zwei
    Klicks erreichbar bleibt (Voraussetzung aus § 5 DDG). ``lang`` kommt
    vom Footer-Link (?lang=de/en), der die gerade eingestellte
    Dashboard-Sprache mitgibt – siehe app.js: spracheAnwenden()."""
    return f"""<!doctype html>
<html lang="{lang}"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{titel} – Brigarde Killfeed</title>
<style>
  :root{{--bg:#12140f;--panel:#20241b;--line:#3a4130;--txt:#e6e9df;--muted:#9aa189;--accent:#7cae3f}}
  ...
  h2{{font-size:16px;margin-top:28px;color:var(--accent)}}
  a{{color:var(--accent)}}
  .zurueck{{display:inline-block;margin-bottom:20px;color:var(--muted);text-decoration:none}}
  .zurueck:hover{{color:var(--accent)}}
  .kasten{{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:16px 20px}}
</style></head>
<body>
<a class="zurueck" href="/">← {zurueck}</a>
<h1>{titel}</h1>
<div class="kasten">{inhalt_html}</div>
</body></html>"""


_IMPRESSUM_DE = """
<p>Angaben gemäß § 5 DDG (Digitale-Dienste-Gesetz)</p>
<p>Deacon Härschel<br>Roermonder Straße 48<br>41812 Erkelenz<br>Deutschland</p>
<h2>Kontakt</h2>
<p>E-Mail: adminbrigardekillfeed@gmail.com</p>
<h2>Verantwortlich für den Inhalt nach § 18 Abs. 2 MStV</h2>
<p>Deacon Härschel (Anschrift wie oben)</p>
<h2>Streitschlichtung</h2>
<p>Die Europäische Kommission stellt eine Plattform zur Online-Streitbeilegung (OS) bereit:
<a href="https://ec.europa.eu/consumers/odr/" target="_blank" rel="noopener noreferrer">
https://ec.europa.eu/consumers/odr/</a>. Ich bin nicht bereit oder verpflichtet, an
...
<p>Deacon Härschel<br>Roermonder Straße 48<br>41812 Erkelenz<br>Germany</p>
<h2>Contact</h2>
<p>Email: adminbrigardekillfeed@gmail.com</p>
<h2>Responsible for content under § 18 Abs. 2 MStV (German Interstate Media Treaty)</h2>
<p>Deacon Härschel (address as above)</p>
<h2>Dispute resolution</h2>
<p>The European Commission provides a platform for online dispute resolution (ODR):
<a href="https://ec.europa.eu/consumers/odr/" target="_blank" rel="noopener noreferrer">
https://ec.europa.eu/consumers/odr/</a>. I am not willing or obliged to participate in
dispute resolution proceedings before a consumer arbitration board.</p>
"""

_DATENSCHUTZ_DE = """
<h2>1. Verantwortlicher</h2>
<p>Deacon Härschel, Roermonder Straße 48, 41812 Erkelenz<br>
E-Mail: adminbrigardekillfeed@gmail.com</p>
<h2>2. Übersicht der Verarbeitungen</h2>
<p>Anmeldung über Discord (OAuth), Bereitstellung des Dashboards und des Discord-Bots,
Server-Logs zur Fehlersuche und Sicherheit.</p>
<h2>3. Hosting</h2>
<p>Dieses Dashboard läuft auf Servern von PebbleHost. Beim Aufruf verarbeitet PebbleHost
technisch notwendige Verbindungsdaten (u. a. IP-Adresse). Es besteht ein
Auftragsverarbeitungsverhältnis nach Art. 28 DSGVO.</p>
<h2>4. Anmeldung über Discord</h2>
<p>Bei der Anmeldung übermittelt Discord deine Discord-ID, deinen Benutzernamen, dein
Profilbild sowie die Discord-Server (Guilds), auf denen du Mitglied bist, an dieses
Dashboard. Diese Daten werden verwendet, um dich anzumelden und dir die Server
anzuzeigen, die du verwalten kannst. Rechtsgrundlage: Art. 6 Abs. 1 lit. b DSGVO
(Erfüllung des Nutzungsverhältnisses).</p>
<h2>5. Cookies / Session</h2>
<p>Zur Anmeldung wird ein technisch notwendiges Session-Cookie gesetzt. Es enthält keine
Trackingfunktion und wird nicht zu Werbezwecken genutzt. Rechtsgrundlage: Art. 6
Abs. 1 lit. f DSGVO (berechtigtes Interesse am Betrieb).</p>
<h2>6. Server-Logs</h2>
<p>Beim Betrieb werden technische Logdaten (u. a. IP-Adresse, Zeitpunkt, aufgerufene
...
<p>Deine Daten werden gespeichert, solange du den Dienst nutzt bzw. dein Server-Zugang
besteht. Auf Wunsch werden sie gelöscht.</p>
...
Daten. Wende dich dazu an die oben genannte E-Mail-Adresse. Außerdem hast du das
Recht, dich bei einer Datenschutz-Aufsichtsbehörde zu beschweren.</p>
"""

_DATENSCHUTZ_EN = """
<h2>1. Controller</h2>
<p>Deacon Härschel, Roermonder Straße 48, 41812 Erkelenz, Germany<br>
Email: adminbrigardekillfeed@gmail.com</p>
<h2>2. Overview of processing</h2>
<p>Login via Discord (OAuth), providing the dashboard and the Discord bot, server logs
for troubleshooting and security.</p>
<h2>3. Hosting</h2>
<p>This dashboard runs on servers provided by PebbleHost. When you access it, PebbleHost
processes technically necessary connection data (including your IP address). A data
processing agreement is in place under Art. 28 GDPR.</p>
<h2>4. Login via Discord</h2>
<p>When you log in, Discord transmits your Discord ID, username, avatar, and the Discord
servers (guilds) you are a member of to this dashboard. This data is used to log you in
and to show you the servers you can manage. Legal basis: Art. 6(1)(b) GDPR (performance
of the usage relationship).</p>
<h2>5. Cookies / session</h2>
<p>A technically necessary session cookie is set on login. It has no tracking function
and is not used for advertising purposes. Legal basis: Art. 6(1)(f) GDPR (legitimate
interest in operating the service).</p>
<h2>6. Server logs</h2>
<p>During operation, technical log data (including IP address, timestamp, requested
address) is stored for a limited time for troubleshooting and abuse prevention. Legal
basis: Art. 6(1)(f) GDPR.</p>
<h2>7. Donations</h2>
<p>Voluntary donations can be made via PayPal (PayPal (Europe) S.à r.l. et Cie, S.C.A.,
22-24 Boulevard Royal, L-2449 Luxembourg). If you choose to donate, PayPal processes the
payment data required for this (including name, payment amount, transaction data); this
processing is carried out under PayPal's own responsibility, based on PayPal's own
privacy policy
(<a href="https://www.paypal.com/en/webapps/mpp/ua/privacy-full" target="_blank"
rel="noopener noreferrer">paypal.com/en/webapps/mpp/ua/privacy-full</a>).
Legal basis on our side: Art. 6(1)(b) GDPR (processing the payment you initiated).</p>
<h2>8. Disclosure to third parties</h2>
<p>Your data is only disclosed where necessary for operation (e.g. to Discord and the
hosting provider), or where required by law.</p>
<h2>9. Storage period</h2>
<p>Your data is stored for as long as you use the service or your server access exists.
It will be deleted on request.</p>
<h2>10. Your rights</h2>
<p>You have the right to access, rectification, erasure, restriction of processing, data
portability, and objection to the processing of your data. Contact the email address
above to exercise these rights. You also have the right to lodge a complaint with a data
protection supervisory authority.</p>
"""

_NUTZUNGSBEDINGUNGEN_DE = """
<h2>1. Geltungsbereich</h2>
<p>Diese Nutzungsbedingungen gelten für die Nutzung des Discord-Bots und
Web-Dashboards „Brigarde Killfeed" (im Folgenden „der Dienst"), betrieben von
Deacon Härschel.</p>
<h2>2. Leistungsbeschreibung</h2>
<p>Der Dienst verbindet einen von dir bereitgestellten Nitrado-DayZ-Server mit Discord
und einem Web-Dashboard, u. a. für Kill-Feeds, Statistiken, Einstellungen und
automatisierte Aufgaben.</p>
<h2>3. Verfügbarkeit</h2>
<p>Der Dienst wird nach bestem Wissen und Gewissen betrieben, es besteht jedoch kein
Anspruch auf ununterbrochene Verfügbarkeit. Wartungsarbeiten, Ausfälle bei
Drittanbietern (Discord, Nitrado, Hosting) oder technische Störungen können die
Nutzung zeitweise einschränken.</p>
<h2>4. Nutzung / Zugangsdaten</h2>
<p>Für die Nutzung stellst du deinen eigenen Nitrado-Zugang bereit. Du bist für die
Sicherheit deiner Zugangsdaten selbst verantwortlich.</p>
<h2>5. Kostenlose Nutzung / Spenden</h2>
<p>Die Nutzung ist derzeit kostenlos. Etwaige Spenden erfolgen freiwillig und ohne
Anspruch auf eine bestimmte Gegenleistung, sofern nicht ausdrücklich anders
vereinbart.</p>
<h2>6. Haftung</h2>
<p>Die Haftung ist, soweit gesetzlich zulässig, auf Vorsatz und grobe Fahrlässigkeit
beschränkt. Für Datenverlust, Schäden am DayZ-Server oder entgangene Spielzeit durch
Fehlfunktionen des Dienstes wird keine Haftung übernommen, soweit kein grobes
Verschulden vorliegt.</p>
<h2>7. Sperrung</h2>
<p>Bei Missbrauch, Verstößen gegen diese Bedingungen oder gegen geltendes Recht kann
der Zugang zum Dienst ohne Vorankündigung eingeschränkt oder gesperrt werden.</p>
<h2>8. Änderungen</h2>
<p>Diese Nutzungsbedingungen können bei Bedarf angepasst werden, etwa wenn sich der
Funktionsumfang des Dienstes ändert. Die jeweils aktuelle Fassung ist unter dieser
Adresse abrufbar.</p>
<h2>9. Anwendbares Recht</h2>
<p>Es gilt deutsches Recht.</p>
"""

_NUTZUNGSBEDINGUNGEN_EN = """
<h2>1. Scope</h2>
<p>These terms of service apply to the use of the Discord bot and web dashboard
"Brigarde Killfeed" (the "Service"), operated by Deacon Härschel.</p>
<h2>2. Description of service</h2>
<p>The Service connects a Nitrado DayZ server you provide with Discord and a web
dashboard, including kill feeds, statistics, settings, and automated tasks.</p>
<h2>3. Availability</h2>
<p>The Service is operated to the best of our knowledge, but there is no entitlement to
uninterrupted availability. Maintenance, outages at third-party providers (Discord,
Nitrado, hosting), or technical faults may temporarily limit use.</p>
<h2>4. Use / credentials</h2>
<p>You provide your own Nitrado access for using the Service. You are responsible for
the security of your own credentials.</p>
<h2>5. Free use / donations</h2>
<p>Use of the Service is currently free of charge. Any donations are made voluntarily
and without entitlement to a specific consideration, unless expressly agreed otherwise.</p>
<h2>6. Liability</h2>
<p>Liability is, to the extent legally permitted, limited to intent and gross
negligence. No liability is assumed for data loss, damage to your DayZ server, or lost
playtime caused by malfunctions of the Service, unless gross fault is involved.</p>
<h2>7. Suspension</h2>
<p>In case of misuse, violations of these terms, or violations of applicable law, access
to the Service may be restricted or suspended without prior notice.</p>
<h2>8. Changes</h2>
<p>These terms of service may be adjusted as needed, e.g. when the scope of the Service
changes. The current version is available at this address.</p>
<h2>9. Applicable law</h2>
<p>German law applies.</p>
"""


def _lang_von(request: web.Request) -> str:
    return "en" if request.query.get("lang") == "en" else "de"


async def _dash_impressum(request: web.Request) -> web.Response:
    lang = _lang_von(request)
    if lang == "en":
        html = _rechtsseite("en", "Imprint", "Back to dashboard", _IMPRESSUM_EN)
    else:
        html = _rechtsseite("de", "Impressum", "Zurück zum Dashboard", _IMPRESSUM_DE)
    return web.Response(text=html, content_type="text/html", charset="utf-8")


async def _dash_datenschutz(request: web.Request) -> web.Response:
    lang = _lang_von(request)
    if lang == "en":
        html = _rechtsseite("en", "Privacy Policy", "Back to dashboard", _DATENSCHUTZ_EN)
    else:
        html = _rechtsseite("de", "Datenschutzerklärung", "Zurück zum Dashboard", _DATENSCHUTZ_DE)
    return web.Response(text=html, content_type="text/html", charset="utf-8")


async def _dash_nutzungsbedingungen(request: web.Request) -> web.Response:
    lang = _lang_von(request)
    if lang == "en":
        html = _rechtsseite("en", "Terms of Service", "Back to dashboard", _NUTZUNGSBEDINGUNGEN_EN)
    else:
        html = _rechtsseite("de", "Nutzungsbedingungen", "Zurück zum Dashboard",
                            _NUTZUNGSBEDINGUNGEN_DE)
    return web.Response(text=html, content_type="text/html", charset="utf-8")
...
def build_app() -> web.Application:
    ...
    r.add_get("/impressum", _dash_impressum)
    r.add_get("/datenschutz", _dash_datenschutz)
    r.add_get("/nutzungsbedingungen", _dash_nutzungsbedingungen)
    ...
    r.add_put("/api/scheduled-tasks/{id}", update_scheduled_task)
...
_ASSET_KNOWN_HASHES: Dict[str, Tuple[str, ...]] = {
    "index.html": (
        ...
        "1b76871698a1446ff44879034c9d7d207b94377ce71c7451767de3c6d9e43623",
    ),
    "styles.css": (
        ...
        "1c780ab5007e4a8a388a33103ab8db4ab1516aecbc23517103701eba86373b4a",
    ...
    "app.js": (
        ...
        "7676b064e8c1d768a4d6af4859986d72fed345e15ba3ad50fd2909f7cbaccad2",
        "086030a4b025cd08c8a73d4c1d9beef49b90694cb4d28615149ce1073932aefe",
        "5f39342e17234f9ae539f57ed635a63e6808b138ce5c9e029853a9ede88d065d",
        "f2cc4a9e3aeb571a3b1c9b94b820d187c6e7cef3f83ca0b44af8bc33a91a2483",
        "2dbc0a2ae742c6e66ccc83ac0e5bf4d6ca9f1a915ffcd5d263a8f1906ec2d8c0",
    ),
    "map.js": (
...
_EMBEDDED_ASSETS: Dict[str, str] = {
    # index.html  (10.591 Bytes roh → 4.488 Bytes base64)
    "index.html": (
        "eNqtWltv3MYVfvevmOxDbaHiruxcmjirLaRIdp3EjhsFDeqXYLicJSdLcgjOUOvVUx7a9K0pkrRF26BuirRIiqIIUKAoUj9V/0R/oP4J/c4Mr3uRVlGMKEty"
        "Zs7MnPPNdy7k8LmDt15756cPD1lkknh0bUg/LOZpuNsLRI8eCB6MrjE2TIThbBzxXAuz2yvMxHu51zSkPBG7vWMpZpnKTY+NVWpEio4zGZhoNxDHciw8e7PN"
        "ZCqN5LGnxzwWuzedGCNNLEYHfP6I7SvDDriOfMXzYDhwLdQnlumU5SLe7Wkzj4WOhMBcUS4mu73BsUgDlQ9iwSexMP2x1r2LB2nDjRwPXMuqMRI7qXoH3PDb"
        "MuGhGOjj8PuPk3h7iAuGi1TvXo+MyW4PBrPZrD97vq/ycHBrZ2eHul5npJl99Xj3+g7bYTd37N/10dCIx4bNd6/3XxHJdTaB0jwtT8Tu9VfQ+uzJL/+B3aPL"
